<?php
	...
	require "connect.php";
	session_start();
	if(!isset($_SESSION['user'])){ header('Location: login.aspx'); }
	...
	$conn = new mysqli($host, $db_user, $db_password);
	$conn->select_db($db_name);
	$sql = "SELECT id FROM KxP";
	$result = $conn->query($sql);
	$in = $result->num_rows;
	$date = new DateTime("now", new DateTimeZone('America/Detroit') );
	$sql = "INSERT INTO `KxP` (`id`, `user`, `content`, `date`, `madeby`) VALUES (".($in+1).", '".$_SESSION['user']."', '".$content."', '". $date->format('Y-m-d H:i:s')."','".$_SESSION['id']."');";
	$result = $conn->query($sql);
	$conn->close();
	header('Location: Default.aspx?feedid=68');
	echo 'success';
	die();
?>